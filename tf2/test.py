...
def detect_face(net, img, scale=1., conf_thresh=0.3):
    # set input x
    if scale != 1:
    ...
    x = tf.convert_to_tensor(img,dtype="float32")
    ...
    bbox_scale = np.array([width, height, width, height])
    priorbox = PriorBox(cfg, image_size=(height, width))
    priors = priorbox.forward()
    boxes = decode(tf.squeeze(loc,axis=0), priors, cfg['variance'])
...
def main(args):
    ...
    for idx in tqdm(range(len(widerface))):
        ...
        dets = detect_face(net, img, conf_thresh=args.confidence_threshold)
        available_scales = get_available_scales(img.shape[0], img.shape[1], scales)
        for available_scale in available_scales:
            det = detect_face(net, img, scale=available_scale, conf_thresh=args.confidence_threshold)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--confidence_threshold', default=0.35, type=float, help='confidence_threshold')
